...
data "archive_file" "lambda_zip" {
  ...
  output_path = var.output_path # naming the output zip file (could be used as a variable to make it dynamic)
}
...
resource "aws_s3_object" "lambda_code" {
  ...
  key    = var.output_path # pointing to the newly created zip file (could be used as a variable)
